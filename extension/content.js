...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message?.type === "BRIDGECART_CAPTURE_PROVIDER_TRACKING") { const captured = globalThis.BridgeCart1688?.captureProviderTracking?.() || { ok: false, code: "ADAPTER_UNAVAILABLE", message: "1688 tracking capture adapter did not load." }; sendResponse({ ...captured, frameUrl: location.href }); return; }
  if (message?.type === "BRIDGECART_CAPTURE_PROVIDER_ORDER") { const adapter = globalThis.BridgeCart1688, captured = adapter?.captureProviderOrder?.(), matches = captured?.ok ? captured.capture.lines.map((line) => adapter.matchCapturedLine(line, message.payload?.queueItems || [])) : [], response = captured?.ok ? { ...captured, matches } : (captured || { ok: false, code: "ADAPTER_UNAVAILABLE", message: "1688 order capture adapter did not load." }); sendResponse({ ...response, frameUrl: location.href }); return; }
