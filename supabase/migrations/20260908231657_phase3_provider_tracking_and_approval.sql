-- A supplier can display the discounted unit price and also show the same
-- discount separately in its order summary. Accept either representation:
-- line subtotals before discount, or final discounted line subtotals.
...
declare v_task public.purchase_tasks%rowtype; v_capture uuid; v_ids uuid[]; v_expected uuid[]; v_line jsonb; v_subtotal numeric; v_final numeric; v_delivery numeric; v_discount numeric;
begin
 ...
 v_final := (p_header->>'finalPaidCny')::numeric; v_delivery := coalesce((p_header->>'domesticDeliveryCny')::numeric,0); v_discount := coalesce((p_header->>'discountCny')::numeric,0);
 if abs(v_final - (v_subtotal - v_discount + v_delivery)) > .01 and abs(v_final - (v_subtotal + v_delivery)) > .01 then raise exception using errcode='23514', message='final paid total does not reconcile to captured SKU lines'; end if;
