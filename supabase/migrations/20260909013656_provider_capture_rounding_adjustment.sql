-- 1688 applies supplier discounts per SKU and rounds each displayed unit price.
-- Preserve the order's exact freight and discount, while permitting only the
-- bounded cent-level residual that this display rounding creates.
...
declare v_task public.purchase_tasks%rowtype; v_capture uuid; v_ids uuid[]; v_expected uuid[]; v_line jsonb; v_subtotal numeric; v_final numeric; v_delivery numeric; v_discount numeric; v_rounding numeric; v_line_count integer;
begin
 ...
 select coalesce(sum((value->>'actualSubtotalCny')::numeric),0), count(*) into v_subtotal, v_line_count from jsonb_array_elements(p_lines);
 v_final := (p_header->>'finalPaidCny')::numeric; v_delivery := coalesce((p_header->>'domesticDeliveryCny')::numeric,0); v_discount := coalesce((p_header->>'discountCny')::numeric,0); v_rounding := coalesce((p_header->>'roundingAdjustmentCny')::numeric,0);
 if abs(v_rounding) > v_line_count * .01 then raise exception using errcode='23514', message='rounding adjustment exceeds one cent per captured SKU line'; end if;
 if abs(v_final - (v_subtotal - v_discount + v_delivery + v_rounding)) > .01 and abs(v_final - (v_subtotal + v_delivery + v_rounding)) > .01 then raise exception using errcode='23514', message='final paid total does not reconcile to captured SKU lines'; end if;
 insert into public.provider_purchase_captures(purchase_task_id,provider_order_id,seller_name,purchased_at,domestic_delivery_cny,discount_cny,final_paid_cny,raw_capture,captured_by) values(v_task.id,p_header->>'providerOrderId',nullif(p_header->>'sellerName',''),nullif(p_header->>'purchasedAt','')::timestamptz,v_delivery,v_discount,v_final,coalesce(p_header->'rawCapture','{}'::jsonb),auth.uid()) returning id into v_capture;
